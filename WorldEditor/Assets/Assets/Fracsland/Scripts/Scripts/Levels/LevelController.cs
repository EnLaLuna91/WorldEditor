...
public class LevelController : MonoBehaviour {
	...
	public void SpamHero(){
        ...
        if (GameObject.FindGameObjectsWithTag("Player").Length > 1) return;
        GameObject h;
        h = GameObject.Find("Hero(Clone)");
        if (h == null)
        {
            GameObject heroPrefab = (GameObject)Resources.Load(Constants.PATH_RESOURCES+"Hero", typeof(GameObject));
            h = (GameObject)Instantiate(heroPrefab, levelStartPoint.transform.position, levelStartPoint.transform.rotation);
            GameObject heroTarget = GameObject.Instantiate((GameObject)Resources.Load(Constants.PATH_RESOURCES + "HeroTarget", typeof(GameObject)));
