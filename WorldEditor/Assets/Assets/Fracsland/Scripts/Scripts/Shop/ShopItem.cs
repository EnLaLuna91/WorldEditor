...
public class ShopItem : MonoBehaviour {
        ...
        if (!owned)
        {
            ...
            switch (_category)
            {
                case Shop.SHOP_CATEGORY.WEAPONS:
                    actionGold = () => {
                        if (Shop.instance.BuyItem(item, GameControllerFracsland.CURRENCY_TYPE.GOLD, true, () => { DataManager.instance.SyncEquippedItem(item.ID, Constants.PLAYER_EQUIPPED_POSITION_WEAPON); }))
                        {
                    ...
                    actionDiammonds = () => {
                        if (Shop.instance.BuyItem(item, GameControllerFracsland.CURRENCY_TYPE.DIAMOND, true, () => { DataManager.instance.SyncEquippedItem(item.ID, Constants.PLAYER_EQUIPPED_POSITION_WEAPON); }))
                        {
                            StatsManager.instance.EquipItem((EquippableItem)item);
                            owned = true;
                            OwnedPresentation();
                ...
                case Shop.SHOP_CATEGORY.HEAD:
                    actionGold = () => {
                        if (Shop.instance.BuyItem(item, GameControllerFracsland.CURRENCY_TYPE.GOLD, true, () => { DataManager.instance.SyncEquippedItem(item.ID, Constants.PLAYER_EQUIPPED_POSITION_HEAD); }))
                        {
                    ...
                    actionDiammonds = () => {
                        if (Shop.instance.BuyItem(item, GameControllerFracsland.CURRENCY_TYPE.DIAMOND, true, () => { DataManager.instance.SyncEquippedItem(item.ID, Constants.PLAYER_EQUIPPED_POSITION_HEAD); }))
                        {
                            StatsManager.instance.EquipHat((EquippableItem)item);
                            owned = true;
                            OwnedPresentation();
                ...
                case Shop.SHOP_CATEGORY.BODY:
                    actionGold = () => {
                        if (Shop.instance.BuyItem(item, GameControllerFracsland.CURRENCY_TYPE.GOLD, true, () => { DataManager.instance.SyncEquippedItem(item.ID, Constants.PLAYER_EQUIPPED_POSITION_SKIN); }))
                        {
                    ...
                    actionDiammonds = () => {
                        if (Shop.instance.BuyItem(item, GameControllerFracsland.CURRENCY_TYPE.DIAMOND, true, () => { DataManager.instance.SyncEquippedItem(item.ID, Constants.PLAYER_EQUIPPED_POSITION_SKIN); }))
                        {
                            StatsManager.instance.EquipSkin((Skin)item);
                            owned = true;
                            OwnedPresentation();
    ...
    void Update()
    {
        ...
        {
            bool hasMoney = false;
            if (GameControllerFracsland.instance.HasEnoughtMoney(item, GameControllerFracsland.CURRENCY_TYPE.GOLD))
            {
            ...
            if (GameControllerFracsland.instance.HasEnoughtMoney(item, GameControllerFracsland.CURRENCY_TYPE.DIAMOND))
            {
